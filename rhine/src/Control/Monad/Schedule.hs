...
--   -- can scheduleT diff be reduced to a?
--   --expects kind (* -> *) -> * -> *
--   wait ::  a -> a m ()
--   wait a = FreeT $ return $ Free $ Wait diff $ return ()
--
-- -- Implement type class MonadSchedule with method race
-- class MonadWait m => MonadSchedule m where
...
-- Make ScheduleT an instance of MonadSchedule
-- instance (MonadSchedule s m) => MonadSchedule s (ScheduleT s m) where
--   race = liftF race
